class Topic < ActiveRecord::Base
  ...
  def newer_topic
    Topic.find(:first, :conditions => ["last_post_at > ?", last_post_at], :order => "last_post_at desc")
  end

  def older_topic
    Topic.find(:first, :conditions => ["last_post_at < ?", last_post_at], :order => "last_post_at asc")
  end
